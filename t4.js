...
function isPlainSafeFirst(c) {
  ...
  return isPrintable(c) && c !== 0xFEFF
    ...
    // - (c-indicator ::=
    // Ã¢â‚¬Å“-Ã¢â‚¬Â | Ã¢â‚¬Å“?Ã¢â‚¬Â | Ã¢â‚¬Å“:Ã¢â‚¬Â | Ã¢â‚¬Å“,Ã¢â‚¬Â | Ã¢â‚¬Å“[Ã¢â‚¬Â | Ã¢â‚¬Å“]Ã¢â‚¬Â | Ã¢â‚¬Å“{Ã¢â‚¬Â | Ã¢â‚¬Å“}Ã¢â‚¬Â
    && c !== CHAR_MINUS
    && c !== CHAR_QUESTION
    && c !== CHAR_COLON
    && c !== CHAR_COMMA
    ...
    && c !== CHAR_SINGLE_QUOTE
    && c !== CHAR_DOUBLE_QUOTE
    // | Ã¢â‚¬Å“%Ã¢â‚¬Â | Ã¢â‚¬Å“@Ã¢â‚¬Â | Ã¢â‚¬Å“`Ã¢â‚¬Â)
    && c !== CHAR_PERCENT
...
function ParseError$1(message, token) {
    ...
    if (token && token.lexer && token.start <= token.end) {
        ...
        var left;
        if (start > 15) {
            left = "Ã¢â‚¬Â¦" + input.slice(start - 15, start);
        } else {
            left = input.slice(0, start);
        }
        var right;
        if (end + 15 < input.length) {
            right = input.slice(end, end + 15) + "Ã¢â‚¬Â¦";
        } else {
...
var iCombinations = {
    ...
    'ÃƒÂ¯': '\u0131\u0308',
    'ÃƒÂ­': '\u0131\u0301',
    // 'Ã„Â«': '\u0131\u0304', // enable when we add Extended Latin
    'ÃƒÂ¬': '\u0131\u0300'
};
...
function symbolNode(value, height, depth, italic, skew, classes, style) {
    ...
    if (/[ÃƒÂ®ÃƒÂ¯ÃƒÂ­ÃƒÂ¬]/.test(this.value)) {    // add Ã„Â« when we add Extended Latin
...
defineSymbol(text, main, textord, "\u2013", "Ã¢â‚¬â€œ");
defineSymbol(text, main, textord, "\u2014", "Ã¢â‚¬â€");
defineSymbol(text, main, textord, "\u2018", "Ã¢â‚¬Ëœ");
defineSymbol(text, main, textord, "\u2019", "Ã¢â‚¬â„¢");
defineSymbol(text, main, textord, "\u201c", "Ã¢â‚¬Å“");
defineSymbol(text, main, textord, "\u201d", "Ã¢â‚¬Â");
});
...
groupTypes.mod = function(group, options) {
    ...
    if (group.value.modType === "bmod") {
        // Ã¢â‚¬Å“\nonscript\mskip-\medmuskip\mkern5muÃ¢â‚¬Â
        if (!options.style.isTight()) {
...
defineFunction([
    "\\dfrac", "\\frac", "\\tfrac",
    "\\dbinom", "\\binom", "\\tbinom",
    "\\\\atopfrac" // canÃ¢â‚¬â„¢t be entered directly
], {
    numArgs: 2,
    greediness: 2
}, function(context, args) {
...
// \left\{\begin{array}{@{}l@{\quad}l@{}} Ã¢â‚¬Â¦ \end{array}\right.
...
/**
 * Given a pair of tokens (this and endToken), compute a Ã¢â‚¬Å“TokenÃ¢â‚¬Â encompassing
...
/**
 * This file contains the Ã¢â‚¬Å“gulletÃ¢â‚¬Â where macros are expanded
        ...
        xhr.onreadystatechange = function () {
          if (xhr.readyState == 4) {
            ...
            if (xhr.status < 400 && xhr.responseText) {
              ...
              code.textContent = 'Ã¢Å“â€“ Error ' + xhr.status + ' while fetching file: ' + xhr.statusText;
            }
            else {
              code.textContent = 'Ã¢Å“â€“ Error: File does not exist or is empty';
            }
          }
        };
...
function punctuation(text){
  ...
  // Elipses
  // text = text.replace(/\.\.\./g,'Ã¢â‚¬Â¦');

  // Nbsp for punc with spaces
  // var NBSP = "\u00a0";
  // var NBSP_PUNCTUATION_START = /([Ã‚Â«Ã‚Â¿Ã‚Â¡]) /g;
  // var NBSP_PUNCTUATION_END = / ([\!\?:;\.,Ã¢â‚¬Â½Ã‚Â»])/g;
...
function quotes(text) {
  ...
  text = text
    .replace(/(\W|^)"([^\s\!\?:;\.,Ã¢â‚¬Â½Ã‚Â»])/g, '$1\u201c$2') // beginning "
